...
namespace Business.Solutions.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : SolutionsServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendPasswordResetLinkAsync(User user, string link = null)
        {
            ...
            if (!link.IsNullOrEmpty())
            {
                ...
                var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                    .ToString(SolutionsConsts.DateTimeOffsetFormat, CultureInfo.InvariantCulture));
